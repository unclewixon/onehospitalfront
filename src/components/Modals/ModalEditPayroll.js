...
class ModalEditPayroll extends Component {
	...
	saveDeductions = async e => {
		...
		const { new_deductions } = this.state;
		const deductions = new_deductions.filter(nd => nd.deleted !== 1).map(p => ({label: p.name, value: p.value}));
		const data = { payslip_id: payroll_id, comment: '', deductions };
		this.setState({ saving: true });
		try {
			console.log(JSON.stringify(data));
			const rs = await request(`${API_URI}${payrollAPI}/update-payslip`, 'PATCH', true, data);
			console.log(rs);
															...
															<tr key={i}>
																<td className="text-left">
																	<div className="form-group text-rightfloat-left pl-0 mb-0">
																		<input className="form-control" placeholder="Deduction" onChange={(e) => this.deductions(e, 'name', d.id)}/>
																	</div>
																</td>
																<td>
																	<div className="row no-gutters">
																		<div className="input-group col-11">
																			...
																			<input className="form-control" type="number" placeholder="Amount" onChange={(e) => this.deductions(e, 'value', d.id)}/>
																		</div>
																		<div className="col-1 text-right">
																			<a className="text-danger" onClick={this.removeDeduction(d.id)} style={{lineHeight: '37px'}}><i className="os-icon os-icon-cancel-circle"/></a>
																		</div>
																	</div>
																</td>
															</tr>
													...
													<tr>
														<td className="text-right mb-3" colSpan="2">
															<div className="text-right mt-2">
																<a className="btn btn-success btn-sm text-white" onClick={this.addDeduction}>
										...
										<div className="row">
											<div className="col-sm-12 text-center mt-4">
												{new_deductions.length > 0 && (
													<button className="btn btn-primary" onClick={this.saveDeductions}>
														{saving ? <img src={waiting} alt="submitting"/> : 'Save'}
													</button>
												)}
