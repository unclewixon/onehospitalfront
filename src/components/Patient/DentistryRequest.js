...
const DentistryRequest = props => {
	...
	const dentistryValue = () => {
		let dentistryServices = props.requestServices
			.filter(service => service.group === 'Dentistry')
			.map(service => {
				return {
					value: service.id,
					label: service.name,
		...
		setDentistryServices(dentistryServices);
	};
