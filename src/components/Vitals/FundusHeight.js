...
const FundusHeight = ({ fullVitals, newVital }) => {
	const [visible, setVisible] = useState(false);
	const [currentVitals, setCurrentVitals] = useState(0);
	useEffect(() => {
		try {
			let v = fullVitals.find(c => c.readingType === info.title);
			setCurrentVitals(v.reading.height);
		} catch (e) {}
	}, [fullVitals]);
	useEffect(() => {
		try {
			setCurrentVitals(newVital.reading.height);
		} catch (e) {}
	}, [newVital]);
	const data = [
	...
	const info = {
		...
		inputs: [{ name: 'height', title: 'Fundus Height', weight: 'cm' }],
	};
		...
		<div className="row vital">
			...
			<div className="col-4">
				<div className="text-center">
					<div className="last-reading">Last Fundus Height Reading:</div>
