...
const Protein = ({ fullVitals, newVital }) => {
	const [visible, setVisible] = useState(false);
	const [currentVitals, setCurrentVitals] = useState(0);
	useEffect(() => {
		try {
			let v = fullVitals.find(c => c.readingType === info.title);
			setCurrentVitals(v.reading.protein);
		} catch (e) {}
	}, [fullVitals]);
	useEffect(() => {
		try {
			setCurrentVitals(newVital.reading.protein);
		} catch (e) {}
	}, [newVital]);
	const data = [
		...
		<div className="row vital">
			...
			<div className="col-4">
				<div className="text-center">
					<div className="last-reading">Last Protein Reading:</div>
